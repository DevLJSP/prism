...
import { PRISM_STDLIB } from "./stdlib";
...
function mapType(t: string | undefined): string {
  ...
  if (genericIdx !== -1) {
    ...
    const args = t.slice(genericIdx);
    const mappedBase = TYPE_MAP[base] ?? base;
    return mappedBase + args;
  }
  return TYPE_MAP[t] ?? t;
}
...
export class CodeGenerator {
  ...
  private stdlibEmitted = new Set<string>();
  ...
  private safeIdent(name: string | undefined): string {
    ...
    if (!out) {
      ...
      if (JS_RESERVED.has(out)) out = `_${out}`;
    }
  ...
  generate(node: ASTNode): string {
    const bodyLines = this.genNode(node);
    const parts: string[] = [];
    if (this.preambles.length > 0) {
      parts.push(this.preambles.join("\n"));
    }
  ...
  private genNode(node: ASTNode): string[] {
    switch (node.type) {
      ...
      case "ImportDeclaration": {
        const { defaultImport, defaultAlias, namedImports, path } = node;

        if (path.startsWith("prism:")) {
          const mod = PRISM_STDLIB[path];
          if (!mod) {
            return [`/* unknown prism stdlib module: ${path} */`];
          }
          if (!this.stdlibEmitted.has(path)) {
            this.preambles.push(mod.preamble);
            this.stdlibEmitted.add(path);
          }
          const lines: string[] = [];
          if (defaultImport) {
      ...
      case "ExportDeclaration": {
        const inner = this.genNode(node.declaration);
        ...
        const trimmed = inner[0].trim();
        return [`export ${trimmed}`, ...inner.slice(1)];
      }
      ...
      case "EnumDeclaration": {
        ...
        lines.push(`${this.ind()}})${closingSuffix}`);
        return lines;
      }
  ...
  genExpr(node: ASTNode): string {
    switch (node.type) {
      ...
      case "ObjectExpression":
        return `{ ${node.properties.map((p) => `${JSON.stringify(p.key)}: ${this.genExpr(p.value)}`).join(", ")} }`;

      case "NewExpression":
        return `new ${this.safeIdent(node.className)}(${node.args.map((a) => this.genExpr(a)).join(", ")})`;
